...
<div>
    ...
    <div x-data="select({ data: {{ $countries }}, value: {{ $country }} , emptyOptionsMessage: '{{ __('No countries match your search.') }}', name: 'country', placeholder: '{{ __('Select a country') }}' })"
        x-init="init()" @click.away="closeListbox()" @keydown.escape="closeListbox()" class="relative flex">
        <span class="inline-block mt-1 rounded-md shadow-sm">
            <button x-ref="button" @click.prevent="toggleListboxVisibility()" :aria-expanded="open"
                aria-haspopup="listbox" style="min-height: 42px;"
                ...
                <div class="flex mr-2">
                    <span x-show="! open"
                        ...
                        :class="{ 'text-gray-500': ! value }" class="block truncate ml-3">asdfas
                    </span>
        ...
        <div x-show="open" x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0" x-cloak class="absolute z-5 w-full mt-1 bg-white rounded-md shadow-lg">
            <input x-ref="search" x-show="open" @keydown.enter.stop.prevent="selectOption()"
                @keydown.arrow-up.prevent="focusPreviousOption()" @keydown.arrow-down.prevent="focusNextOption()"
                x-model="search" type="search" class="w-full z-10 h-full form-control focus:outline-none rounded-md" />
            <ul x-ref="listbox" @keydown.enter.stop.prevent="selectOption()"
                ...
                tabindex="-1" :style="{ 'height': ulSize}"
                ...
                <template x-for="(option, index) in options" :key="index">
                    <li :id="name + 'Option' + focusedOptionOptionId" @click="selectOption()"
                        @mouseenter="focusedOptionOptionId = option.id" @mouseleave="focusedOptionOptionId = null"
                        role="option" :aria-selected="focusedOptionOptionId === option.id"
                        ...
                        <div class="flex items-center">
                            ...
                            <span x-text="`+${option.calling_code}`"
                ...
                <div x-show="! options.length" x-text="emptyOptionsMessage"
                    class="px-3 py-2 text-gray-900 cursor-default select-none"></div>
            </ul>
        </div>
        <x-jet-input type="hidden" id="calling_code" name="calling_code" x-model="value ? value.calling_code : value" />
