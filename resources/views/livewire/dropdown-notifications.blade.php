<div class="xl:flex mt-2 items-center space-x-5 items-center">
    <div x-data="{ dropdownOpen: false }" @click.away="dropdownOpen = false" class="relative my-32">
        ...
        <div x-show="dropdownOpen" class="absolute right-0 rounded-md shadow-lg overflow-hidden z-20" x-cloak
            style="width:20rem;display: none;">
            <div class="bg-gray-100">
                @forelse($notifications as $notification)
                    <a href="#" wire:click="redirectTo('{{ $notification->id }}')"
                        ...
                        <p style="min-width: 18rem;" class=" text-sm mx-2">
                            <span class="font-bold" href="#">{{ $notification->data['title'] }}</span> <small
                                style="float: right"
                                class="float-right">{{ $notification->created_at->diffForHumans() }}</small>
                            <br><span>{{ \Illuminate\Support\Str::limit($notification->data['description'], 48) }}</span>
                        </p>
                    </a>
                @empty
                    <h3 class="mx-auto py-4 text-2xl text-center">{{ __('octo::messages.notifications.all-caught') }}
                    </h3>
                @endforelse
            </div>
            <a href="{{ route('notifications') }}"
                class="block bg-gray-800 text-white text-center font-bold py-2">{{ __('octo::messages.notifications.all-notifications') }}</a>
        </div>
    </div>
</div>
