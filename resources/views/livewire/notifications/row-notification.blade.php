...
<x-livewire-tables::table.cell>
    <div class="px-4 flex justify-end">
        @if ($row->read_at)
            <button wire:click.stop.prevent="markAsUnread('{{ $row->id }}')" wire:loading.attr="disabled"
        ...
        @else
            <button wire:click.stop.prevent="markAsRead('{{ $row->id }}')" wire:loading.attr="disabled"
